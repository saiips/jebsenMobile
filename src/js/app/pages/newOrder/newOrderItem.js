define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'pages/newOrder/newOrderService', 'pages/common/cartService', 'util/commonhelper',
    ...
    'ojs/ojarraytabledatasource', 'ojs/ojpulltorefresh', 'ojs/ojvalidation', 'ojs/ojoffcanvas'],
        function (oj, ko, $, app, service, cartService, commonHelper, amplify, ui, constant) {
            ...
            function NewOrderItemViewModel() {
                var self = this;
                ...
                self.handleActivated = function (info) {
                    ...
                    console.log("newOrderItem.js parentRouter=" + parentRouter.currentState().value);
                    ...
                    var childRouter = parentRouter.getChildRouter("newOrderItem");
                    if (!childRouter) {
                        childRouter = parentRouter.createChildRouter('newOrderItem');
                    }
                    ...
                    if (typeof selCustomer !== "undefined") {
                        ...
                        self.headerTitle(self.lng_newOrder);
                    }
                    ...
                    // set the current state of Navigation Bar (Item List, Order List)
                    self.newOrderNavStateId(app.moduleConfig.params.rootContext.newOrderNavStateId);

                    return oj.Router.sync();
                };

                console.log("NewOrderItemViewModoel");
                ...
                function setupPayloadKey() {
                    ...
                    self.payloadKey(user.username + ":" + orgUnitId + ":" +priceListId);                    
                }
                ...
                function init() {
                    console.log("newOrderItem.js init() started");
                    self.ready(false);
                    ui.showBusy();
                    setupTotalCnt();
                    getPriceListData();
                ...
                function pullToRefresh() {
                    ...
                    service.refreshPriceListMessage(getPayload()).then(cbSuccessFn, cbFailFn);
                }                
...
//                        });
                        ...
                        self.uniquePrincipals = ko.dependentObservable(function () {
                            if (isVanSales()) {
                                var all = [constant.ALL_CODE];
                                principals = all.concat(principals);
                            }
                ...
                function populateSyncDate() {
                    ...
                    var key = constant.STANDARD_PRICE_LIST_KEY + ":" + P_ORG_ID + ":" + P_PRICE_LIST_ID + ":" + constant.NEW_ORDER_LIST_SYNC_DATETIME;
                ...
                self.products = ko.computed(function () {
                    ...
                    if (self.searchText() && allProduct.length > 0 && !self.searchByPrincipal()) {
                        ...
                        var selectItem = ko.utils.unwrapObservable(self.selectItem);
                        var token = new String(selectItem).toLowerCase();
                        var product = filterProductByPrincipal(allProduct, token);
                        return new oj.ArrayTableDataSource(product, {idAttribute: 'id'});
                    }
                });                
                ...
                function filterProductByPrincipal(allProduct, token) {
                    if (token == constant.ALL_CODE.toString().toLowerCase()) {
                        return allProduct;
                    }
                    var product = ko.utils.arrayFilter(allProduct, function (r) {
                ...
                self.checkOut = function (data, event) {
                    ...
                    app.moduleConfig.params.rootContext.fromPage = "newOrderItem";
                ...
                function initTranslation() {
                    ...
                    self.lng_newOrder = getTranslation("ssa.newOrderItem.newOrder");
            ...
            return new NewOrderItemViewModel();
